...
import cookingsFakerFactory from "./cookingsFakerFactory";
...
const CookingsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cookingsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cookings").create(elem));
...
export default connect(null, mapDispatch)(CookingsFakerDialogComponent);
