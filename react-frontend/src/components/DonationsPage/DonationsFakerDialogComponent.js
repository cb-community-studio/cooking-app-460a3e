...
import donationsFakerFactory from "./donationsFakerFactory";
...
const DonationsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = donationsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("donations").create(elem));
...
export default connect(null, mapDispatch)(DonationsFakerDialogComponent);
